...
% Parameters that need to exist but taken from wv_generator
params.scenario = [];        
params.scene_first = [];
params.scene_last = [];
params.active_BS = [];          
...
params.num_ant_TX = [1, 8, 4];      % Number of antenna elements for the radar TX arrays at the BS in the x,y,z-axes
...
params.num_ant_RX = [1, 8, 4];      % Number of antenna elements for the radar RX arrays at the BS in the x,y,z-axes
% To define different array sizes for the selected active TX BSs, you can add multiple rows. 
% Example: For two active BSs with a 8x4 y-z UPA in the first BS and 4x4
% x-z UPA for the second BS, you write  
% params.num_ant_BS = [[1, 8, 4]; [1, 4, 4]];
...
params.radiation_pattern = 0;         % 0: Isotropic and 
                                        % 1: Half-wave dipole
params.radar_channel_taps = 1000; % Radar channel tap length to generate a time domain radar channel impulse response for one "chirp burst"
...
% Channel parameter
params.num_paths = 500;                 % Maximum number of paths to be considered (starting from 1), e.g., choose 1 if you are only interested in the strongest path
