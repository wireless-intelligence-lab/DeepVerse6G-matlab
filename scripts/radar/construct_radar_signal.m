...
function [IF_signal] = construct_radar_signal(tx_ant_size, tx_rotation, tx_FoV, tx_ant_spacing, rx_ant_size, rx_rotation, rx_FoV, rx_ant_spacing, path_params, params)
    ...
    num_paths = sum(path_FoV_filter);
    power = power(path_FoV_filter);
    Doppler_acc = path_params.Doppler_acc(path_FoV_filter);
    Doppler_vel = path_params.Doppler_vel(path_FoV_filter);
    phase = path_params.phase(path_FoV_filter);
    delay_normalized = delay_normalized(path_FoV_filter);
    array_response_TX = array_response_TX(:, path_FoV_filter);
    array_response_RX = array_response_RX(:, path_FoV_filter);
    ...
    if ismember(params.comp_speed, [5, 4, 3]) % Methods 3-5
        ...
        Tau3_rt = ((double(Doppler_acc).*(time_slow.^2))./(2*physconst('LightSpeed')));
        Tau2_rt = ((double(Doppler_vel).*time_slow)./physconst('LightSpeed'));
        ...
        Extra_phase = exp(sqrt(-1)*double(phase).*ang_conv);
