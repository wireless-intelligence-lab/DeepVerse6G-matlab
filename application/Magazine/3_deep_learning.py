...
model.compile(optimizer=opt,
              loss=loss_fn,
              metrics=['accuracy'] + [tf.keras.metrics.TopKCategoricalAccuracy(k=k, name='Top-%i'%k) for k in [1, 2, 3, 4, 5]])
# model.fit(x_train, y_train, batch_size=128, epochs=200)
# model.fit(df_train, validation_data=df_test, batch_size=batch_size, epochs=200)
# model.save_weights('./models/position_noise5')
...
# %%
# model_mixed.fit(df_train_mixed, validation_data=df_test_mixed, batch_size=batch_size, epochs=1)
# results = model_mixed.evaluate(df_test_mixed) 

# model_mixed.save_weights('./models/position_images5')
