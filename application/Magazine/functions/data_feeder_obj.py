...
class DataFeeder(keras.utils.Sequence):
    ...
    def __init__(self, list_IDs, 
        ...
        self.dim = self.data_img.shape[-1]
    ...
    def __data_generation(self, idx):
        ...
        # Initialization
        # if self.img_active:
        #     X_img = np.empty((self.num_cams, self.batch_size, self.dim))
        ...
        #             X_img[j, i,] = self.data_img[j, self.scene_idx_list[ID], :]
            ...
            # y[i] = self.labels[ID]
        if self.img_active:
            X_img = copy.deepcopy(self.data_img[:, self.scene_idx_list[idx], :])
        X_pos = copy.deepcopy(self.data_pos[idx, :])
        y = copy.deepcopy(self.labels[idx])
        if self.img_active:
